...
const displayColumns = [
  {
    ...
    dataIndex: 'id',
    width: '8',
    render: (text, record) => renderTextCell(text, record, 'teacher'),
    sorter: true,
  },
  {
  ...
  return (
    <div key={teacher.id}>
      <DescriptionList key={teacher.id} size="small" col="2">
        ...
        <Description term={fieldLabels.classSize}>
          <div style={{ color: 'red' }}>{teacher.classSize}</div>
        </Description>
        <Description term={fieldLabels.createTime}>
          <div>{moment(teacher.createTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
