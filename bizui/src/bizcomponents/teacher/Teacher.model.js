...
export default {
  ...
  effects: {
    ...
    *addHealthSurveyReport({ payload }, { call, put }) {
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/teacher/${id}/list/HealthSurveyReportList/健康调查报告+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateHealthSurveyReport({ payload }, { call, put }) {
      ...
      const location = { pathname: `/teacher/${id}/list/HealthSurveyReportList/健康调查报告列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
