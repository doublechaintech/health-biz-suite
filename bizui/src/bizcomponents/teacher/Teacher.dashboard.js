...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/teacher/${targetComponent.props.teacher.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/teacher/${targetComponent.props.teacher.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
<Description term="Cq">{teacher.cq==null?appLocaleName(userContext,"NotAssigned"):`${teacher.cq.displayName}(${teacher.cq.id})`}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Cq","changeRequest",TeacherService.requestCandidateCq,
	      TeacherService.transferToAnotherCq,"anotherCqId",teacher.cq?teacher.cq.id:"")} 
