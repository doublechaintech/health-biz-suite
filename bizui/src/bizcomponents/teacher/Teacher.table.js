...
import styles from './Teacher.table.less'
...
class TeacherTable extends PureComponent {
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = TeacherBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/teacher/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
