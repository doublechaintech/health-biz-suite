...
import styles from './SchoolClass.table.less'
...
class SchoolClassTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = SchoolClassBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/schoolClass/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
