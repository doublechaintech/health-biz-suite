...
class UserAnswerCreateFormBody extends Component {
  ...
  render() {
    ...
    const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"CreateNew")}{window.trans('user_answer')}</div>)
    }
      ...
      <div>
        <Card title={!this.props.hideTitle&&appLocaleName(userContext,"BasicInfo")} className={styles.card} bordered={false}>
          <Form >
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.questionAnswer} {...formItemLayout}>
                  {getFieldDecorator('questionAnswer', {
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
                    <Input size="large"  placeHolder={fieldLabels.questionAnswer} />
                  )}
                </Form.Item>
              </Col>
