...
import styles from './ClassQuestion.table.less'
...
class ClassQuestionTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = ClassQuestionBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/classQuestion/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default ClassQuestionTable
