...
const menuData = {
  menuName: window.trans('wechat_workapp_identify'),
  menuFor: 'wechatWorkappIdentify',
  subItems: [],
};
...
const displayColumns = [
  {
    ...
    dataIndex: 'id',
    width: '8',
    render: (text, record) => renderTextCell(text, record, 'wechatWorkappIdentify'),
    sorter: true,
  },
  {
    title: fieldLabels.corpId,
    debugtype: 'string',
    dataIndex: 'corpId',
    width: '18',
    ...
    <div key={wechatWorkappIdentify.id}>
      <DescriptionList key={wechatWorkappIdentify.id} size="small" col="2">
        <Description term={fieldLabels.id} style={{ wordBreak: 'break-all' }}>
          {wechatWorkappIdentify.id}
        </Description>
        <Description term={fieldLabels.corpId} style={{ wordBreak: 'break-all' }}>
          {wechatWorkappIdentify.corpId}
        </Description>
        <Description term={fieldLabels.userId} style={{ wordBreak: 'break-all' }}>
          {wechatWorkappIdentify.userId}
        </Description>
        <Description term={fieldLabels.secUser}>
          <div>
            {wechatWorkappIdentify.secUser == null
              ? appLocaleName(userContext, 'NotAssigned')
              : `${wechatWorkappIdentify.secUser.displayName}(${wechatWorkappIdentify.secUser.id})`}
          </div>
        </Description>
        <Description term={fieldLabels.createTime}>
          <div>{moment(wechatWorkappIdentify.createTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
        <Description term={fieldLabels.lastLoginTime}>
          <div>{moment(wechatWorkappIdentify.lastLoginTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
      </DescriptionList>
...
const WechatWorkappIdentifyBase = {
  menuData,
  displayColumns,
  fieldLabels,
  renderItemOfList,
