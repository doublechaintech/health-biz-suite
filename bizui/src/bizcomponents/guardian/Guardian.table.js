...
import styles from './Guardian.table.less'
...
class GuardianTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = GuardianBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/guardian/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
