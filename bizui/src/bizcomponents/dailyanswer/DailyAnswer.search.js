...
const showAssociateDialog = (targetComponent) => {
  ...
  const { UserDailyAnswerAssociateForm } = GlobalComponents
  const { UserAnswerAssociateForm } = GlobalComponents
  ...
  <div>
    ...
    <UserDailyAnswerAssociateForm 
	visible={currentAssociateModal==='userDailyAnswer'} 
	data={{dailyAnswerList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'userDailyAnswer')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'userDailyAnswer')}/> <UserAnswerAssociateForm 
	visible={currentAssociateModal==='userAnswer'} 
	data={{dailyAnswerList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'userAnswer')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'userAnswer')}/> 
...
class DailyAnswerSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {DailyAnswerTable} = GlobalComponents;
    const {DailyAnswerSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={renderTitle()}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <DailyAnswerSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,DailyAnswerModalTable,"dailyAnswerIds")}
...
export default Form.create()(DailyAnswerSearch)
