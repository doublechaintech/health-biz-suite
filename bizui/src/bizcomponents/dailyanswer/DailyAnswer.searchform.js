...
import styles from './DailyAnswer.search.less'
import GlobalComponents from '../../custcomponents'
import SelectObject from '../../components/SelectObject'
import appLocaleName from '../../common/Locale.tool'
import DailyAnswerBase from './DailyAnswer.base'
const FormItem = Form.Item
const { Option } = Select
const getValue = obj => Object.keys(obj).map(key => obj[key]).join(',')
const {fieldLabels} = DailyAnswerBase
...
class DailyAnswerSearchForm extends PureComponent {
  state = {
    // addInputValue: '',
    // modalVisible: false,
    expandForm: false,
...
componentDidMount() {
  ...
  buildStringSearchParameters = (formValues, fieldName) => {
    ...
    return {
      dailyAnswerList: 1,
      'dailyAnswerList.searchField': fieldName,
      'dailyAnswerList.searchVerb': 'startsWith',
      'dailyAnswerList.searchValue': fieldValue,
    }
  }
  */
  ...
  handleSearch = (e) => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      const {listName} = owner
		...
		pushIfNotNull(paramList,this.buildStringSearchParameters(listName, fieldsValue,'eq', 'userDailyAnswer'))
		pushIfNotNull(paramList,this.buildStringSearchParameters(listName, fieldsValue,'eq', 'userAnswer'))
      ...
      dispatch({
        ...
        dailyAnswerSearchFormParameters: filterObjectKeys(fieldsValue),
 ...
 <Col md={8} sm={24}>
                    <Form.Item label={fieldLabels.userDailyAnswer}>
                  {getFieldDecorator('userDailyAnswer', {initialValue: tryinit('userDailyAnswer')})(
                  
                  <SelectObject 
                    disabled={!availableForEdit('userDailyAnswer')}
                    targetType={"userDailyAnswer"} 
                    requestFunction={DailyAnswerService.requestCandidateUserDailyAnswer} useForSearch />
                  	
                 
                  )}
                </Form.Item></Col>
 <Col md={8} sm={24}>
                    <Form.Item label={fieldLabels.userAnswer}>
                  {getFieldDecorator('userAnswer', {initialValue: tryinit('userAnswer')})(
                  
                  <SelectObject 
                    disabled={!availableForEdit('userAnswer')}
                    targetType={"userAnswer"} 
                    requestFunction={DailyAnswerService.requestCandidateUserAnswer} useForSearch />
...
export default Form.create()(DailyAnswerSearchForm)
