...
import styles from './DailyAnswer.table.less'
...
class DailyAnswerTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = DailyAnswerBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/dailyAnswer/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default DailyAnswerTable
