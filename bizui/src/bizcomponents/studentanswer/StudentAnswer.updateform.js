...
import styles from './StudentAnswer.updateform.less'
...
class StudentAnswerUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const studentAnswerId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, studentAnswerId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateStudentAnswer`,
          payload: {
            id: owner.id,
            type: 'studentAnswer',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextStudentAnswerUpdateRow`,
        payload: {
          id: owner.id,
          type: 'studentAnswer',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}学生回答: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.questionTopic} {...formItemLayout}>
                  {getFieldDecorator('questionTopic', {
                    initialValue: selectedRow.questionTopic,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
...
}))(Form.create()(StudentAnswerUpdateForm))
