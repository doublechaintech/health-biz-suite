...
import styles from './StudentAnswer.search.less'
...
class StudentAnswerSearchForm extends PureComponent {
  state = {
    // addInputValue: '',
    // modalVisible: false,
    expandForm: false,
...
componentDidMount() {
  ...
  buildStringSearchParameters = (formValues, fieldName) => {
    ...
    return {
      studentAnswerList: 1,
      'studentAnswerList.searchField': fieldName,
      'studentAnswerList.searchVerb': 'startsWith',
      'studentAnswerList.searchValue': fieldValue,
    }
  }
  */
  ...
  handleSearch = (e) => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      const {listName} = owner
		...
		pushIfNotNull(paramList,this.buildStringSearchParameters(listName, fieldsValue,'eq', 'healthSurveyReport'))
		pushIfNotNull(paramList,this.buildStringSearchParameters(listName, fieldsValue,'eq', 'dailyAnswer'))
		pushIfNotNull(paramList,this.buildStringSearchParameters(listName, fieldsValue,'contains', 'questionTopic'))
      ...
      dispatch({
        ...
        studentAnswerSearchFormParameters: filterObjectKeys(fieldsValue),
 ...
 <Col md={8} sm={24}>
                 <Form.Item label={fieldLabels.healthSurveyReport}>
               {getFieldDecorator('healthSurveyReport', {initialValue: tryinit('healthSurveyReport')})(
               
               <SelectObject 
                 disabled={!availableForEdit('healthSurveyReport')}
                 targetType={"healthSurveyReport"} 
                 requestFunction={StudentAnswerService.requestCandidateHealthSurveyReport} useForSearch />
 ...
 <Col md={8} sm={24}>
                    <Form.Item label={fieldLabels.dailyAnswer}>
                  {getFieldDecorator('dailyAnswer', {initialValue: tryinit('dailyAnswer')})(
                  
                  <SelectObject 
                    disabled={!availableForEdit('dailyAnswer')}
                    targetType={"dailyAnswer"} 
                    requestFunction={StudentAnswerService.requestCandidateDailyAnswer} useForSearch />
          ...
          <Col md={8} sm={24}>
            <FormItem label={fieldLabels.questionTopic}>
          ...
          <Col md={8} sm={24}>
            <FormItem label={fieldLabels.answer}>
...
export default Form.create()(StudentAnswerSearchForm)
