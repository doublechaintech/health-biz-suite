...
class StudentAnswerTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/studentAnswer/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
