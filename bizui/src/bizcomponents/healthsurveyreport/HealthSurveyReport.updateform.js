...
import styles from './HealthSurveyReport.updateform.less'
...
class HealthSurveyReportUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const healthSurveyReportId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, healthSurveyReportId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateHealthSurveyReport`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextHealthSurveyReportUpdateRow`,
        payload: {
          id: owner.id,
          type: 'healthSurveyReport',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}健康调查报告: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.surveyName} {...formItemLayout}>
                  {getFieldDecorator('surveyName', {
                    initialValue: selectedRow.surveyName,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.teacherName} {...formItemLayout}>
                  {getFieldDecorator('teacherName', {
                    initialValue: selectedRow.teacherName,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
...
}))(Form.create()(HealthSurveyReportUpdateForm))
