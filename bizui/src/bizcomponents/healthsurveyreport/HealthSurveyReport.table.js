...
import styles from './HealthSurveyReport.table.less'
...
class HealthSurveyReportTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = HealthSurveyReportBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/healthSurveyReport/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default HealthSurveyReportTable
