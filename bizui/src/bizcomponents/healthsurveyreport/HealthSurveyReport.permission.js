...
<Description term="调查的时间">{ moment(healthSurveyReport.surveyTime).format('YYYY-MM-DD')}</Description> 
...
const renderPermissionSetting = healthSurveyReport => {
  ...
  return <PermissionSetting targetObject={healthSurveyReport}  targetObjectMeta={HealthSurveyReportBase}/>
}
...
class HealthSurveyReportPermission extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"健康调查报告",cardsFor: "healthSurveyReport",cardsSource: healthSurveyReport,displayName,returnURL,
  		subItems: [
