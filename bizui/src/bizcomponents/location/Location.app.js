...
import styles from './Location.app.less'
...
class LocationBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
	  ...
	  <Menu
       ...
       >
             ...
             <Menu.Item key="dashboard">
               <Link to={`/location/${this.props.location.id}/dashboard`}><Icon type="dashboard" style={{marginRight:"20px"}}/><span>{appLocaleName(userContext,"Dashboard")}</span></Link>
             </Menu.Item>
  ...
  buildRouters = () =>{
  	const {LocationDashboard} = GlobalComponents
  	const {LocationPermission} = GlobalComponents
  	const {LocationProfile} = GlobalComponents
  	
  	
  	const routers=[
  	{path:"/location/:id/dashboard", component: LocationDashboard},
  	{path:"/location/:id/profile", component: LocationProfile},
  	{path:"/location/:id/permission", component: LocationPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.LocationBase
 ...
 <Header style={{ position: 'fixed', zIndex: 1, width: '100%' }}>
        ...
        <Col {...naviBarResponsiveStyle} >
            <Dropdown overlay= {this.getNavMenuItems(this.props.location)}>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.location,this.state.searchKeyword)}</div>
