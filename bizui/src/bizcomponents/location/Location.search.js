...
import styles from './Location.search.less'
...
const showAssociateDialog = (targetComponent) => {
  ...
  const { ProvinceAssociateForm } = GlobalComponents
  ...
  <div>
	...
	data={{locationList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'district')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'district')}/> <ProvinceAssociateForm 
	visible={currentAssociateModal==='province'} 
	data={{locationList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'province')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'province')}/> 
...
class LocationSearch extends PureComponent {
  ...
  render(){
    ...
    const {LocationTable} = GlobalComponents;
    const {LocationSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={renderTitle()}>
        ...
        {showDeletionDialog(this,LocationModalTable,"locationIds")}
...
export default Form.create()(LocationSearch)
