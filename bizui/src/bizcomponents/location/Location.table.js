...
import styles from './Location.table.less'
...
class LocationTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = LocationBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/location/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
