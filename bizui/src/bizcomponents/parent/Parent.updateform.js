...
import styles from './Parent.updateform.less'
...
class ParentUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, parentId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextParentUpdateRow`,
        payload: {
          id: owner.id,
          type: 'parent',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}父: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
