...
import styles from './Parent.table.less'
...
class ParentTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = ParentBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/parent/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
