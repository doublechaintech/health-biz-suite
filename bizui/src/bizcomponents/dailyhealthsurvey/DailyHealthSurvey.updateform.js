...
import styles from './DailyHealthSurvey.updateform.less'
...
class DailyHealthSurveyUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        surveyTime: moment(item.surveyTime),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const dailyHealthSurveyId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, dailyHealthSurveyId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateDailyHealthSurvey`,
          payload: {
            id: owner.id,
            type: 'dailyHealthSurvey',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextDailyHealthSurveyUpdateRow`,
        payload: {
          id: owner.id,
          type: 'dailyHealthSurvey',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}每日健康调查: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.surveyTime} {...formItemLayout}>
                  {getFieldDecorator('surveyTime', {
                    initialValue: selectedRow.surveyTime,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
...
}))(Form.create()(DailyHealthSurveyUpdateForm))
