...
import styles from './DailyHealthSurvey.table.less'
...
class DailyHealthSurveyTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = DailyHealthSurveyBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/dailyHealthSurvey/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default DailyHealthSurveyTable
