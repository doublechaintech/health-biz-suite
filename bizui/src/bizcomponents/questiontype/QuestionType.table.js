...
import styles from './QuestionType.table.less'
...
class QuestionTypeTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = QuestionTypeBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/questionType/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
