...
import styles from './StudentDailyAnswer.table.less'
...
class StudentDailyAnswerTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = StudentDailyAnswerBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/studentDailyAnswer/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default StudentDailyAnswerTable
