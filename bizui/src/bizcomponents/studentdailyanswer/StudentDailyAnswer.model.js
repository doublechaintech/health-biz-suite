...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        const modelName = 'studentDailyAnswer';
  ...
  effects: {
    *view({ payload }, { call, put, select }) {
      const cachedData = yield select(state => state._studentDailyAnswer);
      ...
      const data = yield call(StudentDailyAnswerService.view, payload.id);
    ...
    *load({ payload }, { call, put }) {
      const { StudentDailyAnswerService } = GlobalComponents;
      //yield put({ type: 'showLoading', payload })
      const data = yield call(StudentDailyAnswerService.load, payload.id, payload.parameters);
    ...
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = { pathname: `/studentDailyAnswer/${id}/list/${role}UpdateForm`, state };
    ...
    *addStudentAnswer({ payload }, { call, put }) {
      ...
      const location = {
        pathname: `/studentDailyAnswer/${id}/list/StudentAnswerList/学生回答+${appLocaleName(
          userContext,
          'List'
        )}`,
        state: newState,
    ...
    *updateStudentAnswer({ payload }, { call, put }) {
      ...
      const location = {
        pathname: `/studentDailyAnswer/${id}/list/StudentAnswerList/学生回答列表`,
        state: newPlayload,
      };
      yield put(routerRedux.push(location));
    },
