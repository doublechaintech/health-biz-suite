...
import styles from './StudentDailyAnswer.app.less';
...
class StudentDailyAnswerBizApp extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
    ...
    return (
      ...
      >
        <Menu.Item key="dashboard">
          <Link to={`/studentDailyAnswer/${this.props.studentDailyAnswer.id}/dashboard`}>
  ...
  getStudentAnswerSearch = () => {
    ...
    return connect(state => ({
      ...
      owner: {
        type: '_studentDailyAnswer',
        id: state._studentDailyAnswer.id,
        referenceName: 'dailyAnswer',
        listName: 'studentAnswerList',
        ref: state._studentDailyAnswer,
  ...
  buildRouters = () => {
    const { StudentDailyAnswerDashboard } = GlobalComponents;
    const { StudentDailyAnswerPermission } = GlobalComponents;
    const { StudentDailyAnswerProfile } = GlobalComponents;

    const routers = [
      { path: '/studentDailyAnswer/:id/dashboard', component: StudentDailyAnswerDashboard },
      { path: '/studentDailyAnswer/:id/profile', component: StudentDailyAnswerProfile },
      { path: '/studentDailyAnswer/:id/permission', component: StudentDailyAnswerPermission },

      {
        path: '/studentDailyAnswer/:id/list/studentAnswerList',
        component: this.getStudentAnswerSearch(),
      },
      {
        path: '/studentDailyAnswer/:id/list/studentAnswerCreateForm',
        component: this.getStudentAnswerCreateForm(),
      },
      {
        path: '/studentDailyAnswer/:id/list/studentAnswerUpdateForm',
        component: this.getStudentAnswerUpdateForm(),
      },
      {
        path: '/studentDailyAnswer/:id/ChangeRequestType/:code',
  ...
  render() {
    ...
    const { searchLocalData } = GlobalComponents.StudentDailyAnswerBase;
        ...
        <Header style={{ position: 'fixed', zIndex: 1, width: '100%' }}>
          <Row type="flex" justify="start" align="bottom">
            <Col {...naviBarResponsiveStyle}>
              <Dropdown overlay={this.getNavMenuItems(this.props.studentDailyAnswer)}>
        ...
        <Layout style={{ marginTop: 44 }}>
          {this.state.showSearch && (
            <div style={{ backgroundColor: 'black' }} onClick={() => hideSearchResult()}>
              {searchLocalData(this.props.studentDailyAnswer, this.state.searchKeyword)}
            </div>
          )}
