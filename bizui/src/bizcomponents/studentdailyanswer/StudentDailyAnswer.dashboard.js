...
const internalSettingListOf = studentDailyAnswer =>
  defaultSettingListOf(studentDailyAnswer, optionList);
...
const renderSettingMenuItem = (item, cardsData, targetComponent) => {
  ...
  return (
    <Menu.Item key={item.name}>
      <Link
        to={`/studentDailyAnswer/${targetComponent.props.studentDailyAnswer.id}/list/${item.name}/${
          item.displayName
        }/`}
      >
  ...
  return (
    <Menu>
      <Menu.Item key="profile">
        <Link to={`/studentDailyAnswer/${targetComponent.props.studentDailyAnswer.id}/permission`}>
        ...
        <Icon
          ...
          onClick={() =>
            showTransferModel(
              ...
              '学生健康调查',
              'studentHealthSurvey',
              StudentDailyAnswerService.requestCandidateStudentHealthSurvey,
              StudentDailyAnswerService.transferToAnotherStudentHealthSurvey,
              'anotherStudentHealthSurveyId',
              studentDailyAnswer.studentHealthSurvey
                ? studentDailyAnswer.studentHealthSurvey.id
                : ''
            )
          }
        ...
        <Icon
          ...
          onClick={() =>
            showTransferModel(
              ...
              '检查问题',
              'dailySurveyQuestion',
              StudentDailyAnswerService.requestCandidateQuestion,
              StudentDailyAnswerService.transferToAnotherQuestion,
              'anotherQuestionId',
              studentDailyAnswer.question ? studentDailyAnswer.question.id : ''
            )
          }
          style={{ fontSize: 20, color: 'red' }}
        />
      </Description>
      <Description term="回答" style={{ wordBreak: 'break-all' }}>
        {studentDailyAnswer.answer}
      </Description>
      <Description term="创建时间">
        {moment(studentDailyAnswer.createTime).format('YYYY-MM-DD HH:mm')}
      </Description>
      <Description term="最后更新时间">
        {moment(studentDailyAnswer.lastUpdateTime).format('YYYY-MM-DD HH:mm')}
      </Description>
        ...
        <Icon
          ...
          onClick={() =>
            showTransferModel(
              ...
              '变更请求',
              'changeRequest',
              StudentDailyAnswerService.requestCandidateChangeRequest,
              StudentDailyAnswerService.transferToAnotherChangeRequest,
              'anotherChangeRequestId',
              studentDailyAnswer.changeRequest ? studentDailyAnswer.changeRequest.id : ''
            )
          }
          style={{ fontSize: 20, color: 'red' }}
        />
...
class StudentDailyAnswerDashboard extends Component {
  ...
  render() {
    ...
    const cardsData = {
      ...
      cardsSource: this.props.studentDailyAnswer,
      returnURL,
      displayName,
      subItems: [
        {
