...
const imageList = student => {
  return [{ title: '学生阿凡达', imageLocation: student.studentAvatar }];
};

const internalImageListOf = student => defaultImageListOf(student, imageList);

const optionList = student => {
  return [];
};
...
const renderSettingMenuItem = (item, cardsData, targetComponent) => {
  ...
  return (
    <Menu.Item key={item.name}>
      <Link
        to={`/student/${targetComponent.props.student.id}/list/${item.name}/${item.displayName}/`}
      >
  ...
  return (
    <Menu>
      <Menu.Item key="profile">
        <Link to={`/student/${targetComponent.props.student.id}/permission`}>
    ...
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="地址">
        {student.address == null
          ...
          : `${student.address.displayName}(${student.address.id})`}
        <Icon
          type="swap"
          onClick={() =>
            showTransferModel(
              ...
              '地址',
              'location',
              StudentService.requestCandidateAddress,
              StudentService.transferToAnotherAddress,
              'anotherAddressId',
      ...
      <Description term="用户">
        {student.user == null
          ? appLocaleName(userContext, 'NotAssigned')
          : `${student.user.displayName}(${student.user.id})`}
        <Icon
          type="swap"
          onClick={() =>
            showTransferModel(
              ...
              '用户',
              'user',
              StudentService.requestCandidateUser,
              StudentService.transferToAnotherUser,
              'anotherUserId',
      ...
      <Description term="创建时间">
        {moment(student.createTime).format('YYYY-MM-DD HH:mm')}
      </Description>
      <Description term="变更请求">
        {student.changeRequest == null
          ? appLocaleName(userContext, 'NotAssigned')
          : `${student.changeRequest.displayName}(${student.changeRequest.id})`}
        <Icon
          type="swap"
          onClick={() =>
            showTransferModel(
              ...
              '变更请求',
              'changeRequest',
              StudentService.requestCandidateChangeRequest,
              StudentService.transferToAnotherChangeRequest,
              'anotherChangeRequestId',
              student.changeRequest ? student.changeRequest.id : ''
            )
          }
          style={{ fontSize: 20, color: 'red' }}
        />
...
class StudentDashboard extends Component {
  ...
  render() {
    ...
    const cardsData = {
      cardsName: '学生',
      cardsFor: 'student',
      cardsSource: this.props.student,
      returnURL,
      displayName,
      subItems: [
        {
          ...
          displayName: window.mtrans(
            ...
            'student.student_health_survey_list',
            false
          ),
