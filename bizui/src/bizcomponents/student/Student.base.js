...
const displayColumns = [
  {
    ...
    dataIndex: 'id',
    width: '8',
    render: (text, record) => renderTextCell(text, record, 'student'),
    sorter: true,
  },
  {
  ...
  {
    title: fieldLabels.studentAvatar,
    dataIndex: 'studentAvatar',
    render: (text, record) => renderAvatarCell(text, record, 'student.student_avatar'),
  },
  {
  ...
  return (
    <div key={student.id}>
      <DescriptionList key={student.id} size="small" col="2">
        ...
        <Description term={fieldLabels.user}>
          <div>
            {student.user == null
              ? appLocaleName(userContext, 'NotAssigned')
        ...
        <Description term={fieldLabels.createTime}>
          <div>{moment(student.createTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
