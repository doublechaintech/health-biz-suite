...
import styles from './Student.table.less'
...
class StudentTable extends PureComponent {
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = StudentBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/student/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
