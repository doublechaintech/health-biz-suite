  ...
  return (
    <div>
      <LocationAssociateForm
        ...
        data={{ studentList: selectedRows }}
        owner={owner}
        onCancel={() => toggleAssociateModalVisible(targetComponent, 'address')}
        onCreate={() => toggleAssociateModalVisible(targetComponent, 'address')}
      />
      <UserAssociateForm
...
class StudentSearch extends PureComponent {
  ...
  render() {
    ...
    const { StudentTable } = GlobalComponents;
    const { StudentSearchForm } = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={renderTitle()}>
        ...
        {showDeletionDialog(this, StudentModalTable, 'studentIds')}
