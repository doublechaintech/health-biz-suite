...
import styles from './Student.updateform.less'
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, studentId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextStudentUpdateRow`,
        payload: {
          id: owner.id,
          type: 'student',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}学生: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
