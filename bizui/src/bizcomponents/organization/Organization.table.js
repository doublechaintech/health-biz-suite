...
import styles from './Organization.table.less'
...
class OrganizationTable extends PureComponent {
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = OrganizationBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/organization/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
