...
import styles from './ClassDailyHealthSurvey.table.less'
...
class ClassDailyHealthSurveyTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = ClassDailyHealthSurveyBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/classDailyHealthSurvey/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default ClassDailyHealthSurveyTable
