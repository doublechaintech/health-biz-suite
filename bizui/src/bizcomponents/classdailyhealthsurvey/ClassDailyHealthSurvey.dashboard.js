...
import styles from './ClassDailyHealthSurvey.dashboard.less';
...
const imageList = classDailyHealthSurvey => {
  return [];
};

const internalImageListOf = classDailyHealthSurvey =>
  defaultImageListOf(classDailyHealthSurvey, imageList);

const optionList = classDailyHealthSurvey => {
  return [];
};
...
const internalSettingListOf = classDailyHealthSurvey =>
  defaultSettingListOf(classDailyHealthSurvey, optionList);
const internalLargeTextOf = classDailyHealthSurvey => {
  return null;
};
...
const renderSettingMenuItem = (item, cardsData, targetComponent) => {
  ...
  return (
    <Menu.Item key={item.name}>
      <Link
        to={`/classDailyHealthSurvey/${targetComponent.props.classDailyHealthSurvey.id}/list/${
          item.name
        }/${item.displayName}/`}
      >
  ...
  return (
    <Menu>
      <Menu.Item key="profile">
        <Link
          to={`/classDailyHealthSurvey/${
            targetComponent.props.classDailyHealthSurvey.id
          }/permission`}
        >
    ...
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="老师">
        {classDailyHealthSurvey.teacher == null
          ...
          : `${classDailyHealthSurvey.teacher.displayName}(${classDailyHealthSurvey.teacher.id})`}
        <Icon
          type="swap"
          onClick={() =>
            showTransferModel(
              ...
              '老师',
              'teacher',
              ClassDailyHealthSurveyService.requestCandidateTeacher,
              ClassDailyHealthSurveyService.transferToAnotherTeacher,
              'anotherTeacherId',
              classDailyHealthSurvey.teacher ? classDailyHealthSurvey.teacher.id : ''
            )
          }
      ...
      <Description term="调查的时间">
        {moment(classDailyHealthSurvey.surveyTime).format('YYYY-MM-DD HH:mm')}
      </Description>
      <Description term="创建人名称">
        {classDailyHealthSurvey.creator == null
          ? appLocaleName(userContext, 'NotAssigned')
          : `${classDailyHealthSurvey.creator.displayName}(${classDailyHealthSurvey.creator.id})`}
        <Icon
          type="swap"
          onClick={() =>
            showTransferModel(
              ...
              '创建人名称',
              'user',
              ClassDailyHealthSurveyService.requestCandidateCreator,
              ClassDailyHealthSurveyService.transferToAnotherCreator,
              'anotherCreatorId',
              classDailyHealthSurvey.creator ? classDailyHealthSurvey.creator.id : ''
            )
          }
          style={{ fontSize: 20, color: 'red' }}
        />
        ...
        <Icon
          ...
          onClick={() =>
            showTransferModel(
              ...
              '变更请求',
              'changeRequest',
              ClassDailyHealthSurveyService.requestCandidateChangeRequest,
              ClassDailyHealthSurveyService.transferToAnotherChangeRequest,
              'anotherChangeRequestId',
              classDailyHealthSurvey.changeRequest ? classDailyHealthSurvey.changeRequest.id : ''
            )
          }
          style={{ fontSize: 20, color: 'red' }}
        />
      ...
      {buildTransferModal(classDailyHealthSurvey, targetComponent)}
    </DescriptionList>
  );
};
...
class ClassDailyHealthSurveyDashboard extends Component {
  state = {
    ...
    defaultType: 'classDailyHealthSurvey',
  };
  componentDidMount() {}
  ...
  render() {
    ...
    const {
      id,
      displayName,
      dailySurveyQuestionListMetaInfo,
    ...
    } = this.props.classDailyHealthSurvey;
    ...
    const cardsData = {
      ...
      cardsSource: this.props.classDailyHealthSurvey,
      returnURL,
      displayName,
      subItems: [
        {
          ...
          count: dailySurveyQuestionCount,
          addFunction: true,
          role: 'dailySurveyQuestion',
          metaInfo: dailySurveyQuestionListMetaInfo,
          renderItem: GlobalComponents.DailySurveyQuestionBase.renderItemOfList,
        },
        {
...
}))(Form.create()(ClassDailyHealthSurveyDashboard));
