...
const packFormValuesToObject = ( formValuesToPack )=>{
	const {name, surveyTime, teacherId, creatorId, cqId} = formValuesToPack
	const teacher = {id: teacherId, version: 2^31}
	const creator = {id: creatorId, version: 2^31}
	const cq = {id: cqId, version: 2^31}
	const data = {name, surveyTime, teacher, creator, cq}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {name, surveyTime, teacher, creator, cq} = objectToUnpack
	const teacherId = teacher ? teacher.id : null
	const creatorId = creator ? creator.id : null
	const cqId = cq ? cq.id : null
	const data = {name, surveyTime, teacherId, creatorId, cqId}
	return data
}
