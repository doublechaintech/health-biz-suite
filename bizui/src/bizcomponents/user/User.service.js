...
const addStudent = (targetObjectId, parameters) => {
  const url = `${PREFIX}userManager/addStudent/userId/studentName/studentNumber/studentAvatar/guardianName/guardianMobile/addressId/platformId/changeRequestId/tokensExpr/`;
  const userId = targetObjectId;
  const requestParameters = { ...parameters, userId, tokensExpr: 'none' };
  return postForm({ url, requestParameters });
};
