...
const displayColumns = [
  {
    ...
    dataIndex: 'id',
    width: '8',
    render: (text, record) => renderTextCell(text, record, 'user'),
    sorter: true,
  },
  {
  ...
  {
    title: fieldLabels.avatar,
    dataIndex: 'avatar',
    render: (text, record) => renderAvatarCell(text, record, 'user.avatar'),
  },
  {
    ...
    <div key={user.id}>
      <DescriptionList key={user.id} size="small" col="2">
        ...
        <Description term={fieldLabels.createTime}>
          <div>{moment(user.createTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
      </DescriptionList>
