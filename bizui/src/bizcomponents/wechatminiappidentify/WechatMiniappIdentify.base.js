...
const menuData = {
  menuName: window.trans('wechat_miniapp_identify'),
  menuFor: 'wechatMiniappIdentify',
  subItems: [],
};
...
const displayColumns = [
  {
    ...
    dataIndex: 'id',
    width: '8',
    render: (text, record) => renderTextCell(text, record, 'wechatMiniappIdentify'),
    sorter: true,
  },
  {
    title: fieldLabels.openId,
    debugtype: 'string',
    dataIndex: 'openId',
    width: '29',
    ...
    <div key={wechatMiniappIdentify.id}>
      <DescriptionList key={wechatMiniappIdentify.id} size="small" col="2">
        <Description term={fieldLabels.id} style={{ wordBreak: 'break-all' }}>
          {wechatMiniappIdentify.id}
        </Description>
        <Description term={fieldLabels.openId} style={{ wordBreak: 'break-all' }}>
          {wechatMiniappIdentify.openId}
        </Description>
        <Description term={fieldLabels.appId} style={{ wordBreak: 'break-all' }}>
          {wechatMiniappIdentify.appId}
        </Description>
        <Description term={fieldLabels.secUser}>
          <div>
            {wechatMiniappIdentify.secUser == null
              ? appLocaleName(userContext, 'NotAssigned')
              : `${wechatMiniappIdentify.secUser.displayName}(${wechatMiniappIdentify.secUser.id})`}
          </div>
        </Description>
        <Description term={fieldLabels.createTime}>
          <div>{moment(wechatMiniappIdentify.createTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
        <Description term={fieldLabels.lastLoginTime}>
          <div>{moment(wechatMiniappIdentify.lastLoginTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
      </DescriptionList>
...
const WechatMiniappIdentifyBase = {
  menuData,
  displayColumns,
  fieldLabels,
  renderItemOfList,
