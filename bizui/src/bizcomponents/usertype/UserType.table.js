...
import styles from './UserType.table.less'
...
class UserTypeTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = UserTypeBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/userType/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
