...
import styles from './UserDailyAnswer.updateform.less'
...
class UserDailyAnswerUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        answerTime: moment(item.answerTime),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const userDailyAnswerId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, userDailyAnswerId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateUserDailyAnswer`,
          payload: {
            id: owner.id,
            type: 'userDailyAnswer',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextUserDailyAnswerUpdateRow`,
        payload: {
          id: owner.id,
          type: 'userDailyAnswer',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}用户每日回答: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.answerTime} {...formItemLayout}>
                  {getFieldDecorator('answerTime', {
                    initialValue: selectedRow.answerTime,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.isAnswerSubmitted} {...switchFormItemLayout}>
                  {getFieldDecorator('isAnswerSubmitted', {
                    initialValue: selectedRow.isAnswerSubmitted,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
...
}))(Form.create()(UserDailyAnswerUpdateForm))
