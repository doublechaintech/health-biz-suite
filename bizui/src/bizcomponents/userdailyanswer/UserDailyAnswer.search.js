...
import styles from './UserDailyAnswer.search.less'
...
const showAssociateDialog = (targetComponent) => {
  ...
  const { DailyHealthSurveyAssociateForm } = GlobalComponents
  ...
  <div>
	...
	data={{userDailyAnswerList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'user')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'user')}/> <OrganizationAssociateForm 
	visible={currentAssociateModal==='organization'} 
	data={{userDailyAnswerList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'organization')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'organization')}/> <DailyHealthSurveyAssociateForm 
	visible={currentAssociateModal==='dailyHealthSurvey'} 
	data={{userDailyAnswerList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'dailyHealthSurvey')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'dailyHealthSurvey')}/> 
...
class UserDailyAnswerSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {UserDailyAnswerTable} = GlobalComponents;
    const {UserDailyAnswerSearchForm} = GlobalComponents;
    const {UserDailyAnswerModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={renderTitle()}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <UserDailyAnswerSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,UserDailyAnswerModalTable,"userDailyAnswerIds")}
...
export default Form.create()(UserDailyAnswerSearch)
