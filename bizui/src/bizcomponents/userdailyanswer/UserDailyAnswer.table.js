...
import styles from './UserDailyAnswer.table.less'
...
class UserDailyAnswerTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = UserDailyAnswerBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/userDailyAnswer/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default UserDailyAnswerTable
