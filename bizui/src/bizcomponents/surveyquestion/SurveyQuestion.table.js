...
import styles from './SurveyQuestion.table.less'
...
class SurveyQuestionTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = SurveyQuestionBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/surveyQuestion/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
