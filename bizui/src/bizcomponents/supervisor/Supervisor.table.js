...
import styles from './Supervisor.table.less'
...
class SupervisorTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = SupervisorBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/supervisor/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
