...
const addStudent = (targetObjectId, parameters) => {
  const url = `${PREFIX}platformManager/addStudent/platformId/studentName/studentNumber/studentAvatar/guardianName/guardianMobile/addressId/userId/changeRequestId/tokensExpr/`;
  const platformId = targetObjectId;
  const requestParameters = { ...parameters, platformId, tokensExpr: 'none' };
  return postForm({ url, requestParameters });
};
