...
const menuData = {
  menuName: window.trans('student_health_survey'),
  menuFor: 'studentHealthSurvey',
  subItems: [
    {
      name: 'studentDailyAnswerList',
      displayName: window.mtrans(
        'student_daily_answer',
        'student_health_survey.student_daily_answer_list',
        false
      ),
      type: 'studentDailyAnswer',
      icon: '500px',
...
const fieldLabels = {
  ...
  changeRequest: window.trans('student_health_survey.change_request'),
};
...
const displayColumns = [
  {
    ...
    dataIndex: 'id',
    width: '8',
    render: (text, record) => renderTextCell(text, record, 'studentHealthSurvey'),
    sorter: true,
  },
  {
  ...
  {
    title: fieldLabels.answerTime,
    dataIndex: 'answerTime',
    render: (text, record) => renderDateTimeCell(text, record),
    sorter: true,
  },
  {
    title: fieldLabels.surveyStatus,
  ...
  {
    title: fieldLabels.classDailyHealthSurvey,
    dataIndex: 'classDailyHealthSurvey',
    render: (text, record) => renderReferenceCell(text, record),
    sorter: true,
  },
  {
    ...
    <div key={studentHealthSurvey.id}>
      <DescriptionList key={studentHealthSurvey.id} size="small" col="2">
        <Description term={fieldLabels.id} style={{ wordBreak: 'break-all' }}>
          {studentHealthSurvey.id}
        </Description>
        <Description term={fieldLabels.student}>
          <div>
            {studentHealthSurvey.student == null
              ? appLocaleName(userContext, 'NotAssigned')
              : `${studentHealthSurvey.student.displayName}(${studentHealthSurvey.student.id})`}
          </div>
        </Description>
        <Description term={fieldLabels.answerTime}>
          <div>{moment(studentHealthSurvey.answerTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
        <Description term={fieldLabels.surveyStatus}>
          <div>
            {studentHealthSurvey.surveyStatus == null
              ? appLocaleName(userContext, 'NotAssigned')
              : `${studentHealthSurvey.surveyStatus.displayName}(${
                  studentHealthSurvey.surveyStatus.id
                })`}
          </div>
        </Description>
        <Description term={fieldLabels.teacher}>
          <div>
            {studentHealthSurvey.teacher == null
              ? appLocaleName(userContext, 'NotAssigned')
              : `${studentHealthSurvey.teacher.displayName}(${studentHealthSurvey.teacher.id})`}
          </div>
        </Description>
        <Description term={fieldLabels.classDailyHealthSurvey}>
          <div>
            {studentHealthSurvey.classDailyHealthSurvey == null
              ? appLocaleName(userContext, 'NotAssigned')
              : `${studentHealthSurvey.classDailyHealthSurvey.displayName}(${
                  studentHealthSurvey.classDailyHealthSurvey.id
                })`}
          </div>
        </Description>
        <Description term={fieldLabels.createTime}>
          <div>{moment(studentHealthSurvey.createTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
        <Description term={fieldLabels.lastUpdateTime}>
          <div>{moment(studentHealthSurvey.lastUpdateTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
        <Description term={fieldLabels.changeRequest}>
          <div>
            {studentHealthSurvey.changeRequest == null
              ? appLocaleName(userContext, 'NotAssigned')
              : `${studentHealthSurvey.changeRequest.displayName}(${
                  studentHealthSurvey.changeRequest.id
                })`}
          </div>
        </Description>
      </DescriptionList>
...
const packFormValuesToObject = formValuesToPack => {
  ...
  const data = {
    answerTime: moment(answerTime).valueOf(),
    student,
    surveyStatus,
    teacher,
