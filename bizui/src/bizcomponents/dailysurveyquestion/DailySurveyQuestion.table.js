...
import styles from './DailySurveyQuestion.table.less'
...
class DailySurveyQuestionTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = DailySurveyQuestionBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/dailySurveyQuestion/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default DailySurveyQuestionTable
