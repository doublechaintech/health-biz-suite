...
<c:if test="${ empty teacherList}" >
	<div class="row" style="font-size: 30px;">
		<div class="col-xs-12 col-md-12" style="padding-left:20px">
		 ${userContext.localeMap['@not_found']}${userContext.localeMap['teacher']}! 
		 <a href="./${ownerBeanName}Manager/addTeacher/${result.id}/"><i class="fa fa-plus-square" aria-hidden="true"></i></a>
...
<%
 	...
 	SmartList list=(SmartList)request.getAttribute("teacherList"); 
...
%>
    ...
    <div class="row" style="font-size: 30px;">
		<div class="col-xs-12 col-md-12" style="padding-left:20px">
		<i class='fa fa-table'></i> ${userContext.localeMap['teacher']}(${totalCount})
		<a href="./${ownerBeanName}Manager/addTeacher/${result.id}/"><i class="fa fa-plus-square" aria-hidden="true"></i></a>
		 
		 		 	<div class="btn-group" role="group" aria-label="Button group with nested dropdown">		
	<c:forEach var="action" items="${result.actionList}">
		<c:if test="${'teacherList' eq action.actionGroup}">
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${teacherListName};${teacherListName}CurrentPage=${page.pageNumber};${teacherListName}RowsPerPage=${rowsPerPage}/' 
	...
	<table class="table table-striped" pageToken='teacherListCurrentPage=${currentPageNumber}'>
		<thead><tr>
		<c:if test="${param.referName ne 'id'}">
	<th>${userContext.localeMap['teacher.id']}</th>
</c:if>
<c:if test="${param.referName ne 'name'}">
	<th>${userContext.localeMap['teacher.name']}</th>
</c:if>
...
<c:if test="${param.referName ne 'classSize'}">
	<th>${userContext.localeMap['teacher.class_size']}</th>
</c:if>
...
<c:if test="${param.referName ne 'user'}">
	<th>${userContext.localeMap['teacher.user']}</th>
</c:if>
...
<c:if test="${param.referName ne 'name'}">	<td contenteditable='true' class='edit-value'  propertyToChange='name' storedCellValue='${item.name}' prefix='${ownerBeanName}Manager/updateTeacher/${result.id}/${item.id}/'>${item.name}</td>
...
</c:if><c:if test="${param.referName ne 'classSize'}">	<td contenteditable='true' class='edit-value'  propertyToChange='classSize' storedCellValue='${item.classSize}' prefix='${ownerBeanName}Manager/updateTeacher/${result.id}/${item.id}/'>${item.classSize}</td>
	...
	<td class="select_candidate_td"
			data-candidate-method="./teacherManager/requestCandidatePlatform/${ownerBeanName}/${item.id}/"
			data-switch-method="./teacherManager/transferToAnotherPlatform/${item.id}/"
...
<c:if test="${param.referName ne 'user'}">
	<td class="select_candidate_td"
			data-candidate-method="./teacherManager/requestCandidateUser/${ownerBeanName}/${item.id}/"
			data-switch-method="./teacherManager/transferToAnotherUser/${item.id}/"
	...
	<td class="select_candidate_td"
		...
		<div class="candidate_span" style="display:none;">
			<input type="text" data-provide="typeahead" class="input-sm form-control candidate-filter-input" autocomplete="off" />
				...
				<a href='#${ownerBeanName}Manager/removeTeacher/${result.id}/${item.id}/' class='delete-action btn btn-danger btn-xs'><i class="fa fa-trash-o fa-lg"></i> ${userContext.localeMap['@delete']}</a>
				<a href='#${ownerBeanName}Manager/copyTeacherFrom/${result.id}/${item.id}/' class='copy-action btn btn-success btn-xs'><i class="fa fa-files-o fa-lg"></i> ${userContext.localeMap['@copy']} </a>
