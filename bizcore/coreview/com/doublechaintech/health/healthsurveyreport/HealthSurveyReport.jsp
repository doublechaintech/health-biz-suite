<%@ page import='java.util.*,com.doublechaintech.health.*,com.doublechaintech.health.healthsurveyreport.HealthSurveyReport'%>
...
<c:set var="ownerBeanName" value="healthSurveyReport" scope="request"/>
<c:set var="ownerClassName" value="HealthSurveyReport" scope="request"/>
...
<div class="row">
	...
	<% HealthSurveyReport result = (HealthSurveyReport)request.getAttribute("result");  %>
			<li><a data-toggle="tab" href="#studentAnswerList" class="disabled"> ${userContext.localeMap['student_answer']}</a></li>
...
<div class="row" desc="show parent objects">
	...
	<c:set var="healthSurveyReport" value="${ result}" scope="request" />
<sky:include page="com/doublechaintech/health/healthsurveyreport/HealthSurveyReport$Summary.jsp" />
	...
	<div class="row">
<c:set var="student" value="${ result.student}" scope="request" />
<c:set var="referName" value="" scope="request" />
<sky:include page="com/doublechaintech/health/student/Student$Info.jsp" />

<c:set var="teacher" value="${ result.teacher}" scope="request" />
<c:set var="referName" value="" scope="request" />
<sky:include page="com/doublechaintech/health/teacher/Teacher$Info.jsp" />

<c:set var="classDailyHealthSurvey" value="${ result.survey}" scope="request" />
<c:set var="referName" value="(survey)" scope="request" />
<sky:include page="com/doublechaintech/health/classdailyhealthsurvey/ClassDailyHealthSurvey$Info.jsp" />
		...
		<c:if test='${not empty userContext.accessTokens["studentAnswerList"] or ignoreListAccessControl}'>
		<c:set var="studentAnswerList" value="${result.studentAnswerList}" scope="request"/>
		<c:set var="studentAnswerListName" value="studentAnswerList" scope="request"/>
		<div id="studentAnswerList" class="tab-pane fade sublist" refer-name="health_survey_report">
			<sky:include page="com/doublechaintech/health/studentanswer/StudentAnswer$List.jsp"
					referName="healthSurveyReport"/>
		</div>
	</c:if>
