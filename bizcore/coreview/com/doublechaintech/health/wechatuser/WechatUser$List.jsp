...
<c:if test="${param.referName ne 'avatar'}">
	<th>${userContext.localeMap['wechat_user.avatar']}</th>
</c:if>
<c:if test="${param.referName ne 'address'}">
	<th>${userContext.localeMap['wechat_user.address']}</th>
</c:if>
...
</c:if><c:if test="${param.referName ne 'avatar'}">	<td contenteditable='true' class='edit-value'  propertyToChange='avatar' storedCellValue='${item.avatar}' prefix='${ownerBeanName}Manager/updateWechatUser/${result.id}/${item.id}/'>${item.avatar}</td>
</c:if><c:if test="${param.referName ne 'address'}">
	<td class="select_candidate_td"
			data-candidate-method="./wechatUserManager/requestCandidateAddress/${ownerBeanName}/${item.id}/"
			data-switch-method="./wechatUserManager/transferToAnotherAddress/${item.id}/"
...
<c:if test="${param.referName ne 'userType'}">
	<td class="select_candidate_td"
			data-candidate-method="./wechatUserManager/requestCandidateUserType/${ownerBeanName}/${item.id}/"
			data-switch-method="./wechatUserManager/transferToAnotherUserType/${item.id}/"
			data-link-template="./userTypeManager/view/${'$'}{ID}/">
		<span class="display_span">
			<c:if test="${not empty  item.userType}">
			<a href='./userTypeManager/view/${item.userType.id}/'>${item.userType.displayName}</a>
			</c:if>
			<c:if test="${empty  item.userType}">
			<a href='#'></a>
			</c:if>
			<button class="btn btn-link candidate-action">...</button>
		</span>
...
<c:if test="${param.referName ne 'createTime'}">	<td contenteditable='true' class='edit-value'  propertyToChange='createTime' storedCellValue='${item.createTime}' prefix='${ownerBeanName}Manager/updateWechatUser/${result.id}/${item.id}/'><fmt:formatDate pattern="yyyy-MM-dd'T'HH:mm:ss" value="${item.createTime}" /></td>
