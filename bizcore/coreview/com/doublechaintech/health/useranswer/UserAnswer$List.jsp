...
<c:if test="${param.referName ne 'questionAnswer'}">
	<th>${userContext.localeMap['user_answer.question_answer']}</th>
</c:if>
...
<c:if test="${param.referName ne 'questionAnswer'}">	<td contenteditable='true' class='edit-value'  propertyToChange='questionAnswer' storedCellValue='${item.questionAnswer}' prefix='${ownerBeanName}Manager/updateUserAnswer/${result.id}/${item.id}/'>${item.questionAnswer}</td>
</c:if>
				<td>
