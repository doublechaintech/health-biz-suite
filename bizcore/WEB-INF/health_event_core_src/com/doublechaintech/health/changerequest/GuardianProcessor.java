...
public class GuardianProcessor extends ChangeRequestChainProcessor{
	
	
	protected void processInternal(HealthUserContext userContext, ChangeRequest request, String beanName){
		request.getGuardianList().forEach(event->{
	...
	protected void handleSingleEvent(HealthUserContext userContext, ChangeRequest request, Guardian event ){
		...
		userContext.log("GuardianProcessor\t"+ event +" from processor");
