...
public class HealthBaseUtils {
	...
	public static String getCacheAccessKey(HealthUserContext ctx) {
	...
	protected static BaseEntity loadCanCacheInLocal(HealthUserContext userContext, String type, String id) throws Exception {
	...
	public static <T extends BaseEntity> List<T> collectReferencedObjectWithType(HealthUserContext userContext,
	...
	private static <T extends BaseEntity> void collectReferedObjectIdSet(HealthUserContext userContext,
	...
	public static int getAppBuildVersion(HealthUserContext ctx) {
		return getBuildVersion(getRequestAppVersion(ctx));
	}
	protected static boolean startFromVersion(HealthUserContext ctx, int version) {
     ...
     * "x-app-version" : "9"
	 */
	public static String getRequestAppVersion(HealthUserContext userContext) {
		return userContext.getRequestHeader("x-app-version");
	}
	public static String getRequestAppDevice(HealthUserContext userContext) {
		return userContext.getRequestHeader("x-app-device");
	}
	public static String getRequestAppType(HealthUserContext userContext) {
	...
	public static QuestionType getQuestionType(HealthUserContext userContext, String code) throws Exception {
		String key = "enum:" + QuestionType.INTERNAL_TYPE + ":" + code;
		QuestionType data = (QuestionType) userContext.getFromContextLocalStorage(key);
		if (data != null) {
			return data;
		}
		data = userContext.getDAOGroup().getQuestionTypeDAO().loadByCode(code, emptyOptions);
	...
	public static ChangeRequestType getChangeRequestType(HealthUserContext userContext, String code) throws Exception {
	...
	public static <T> T loadBaseEntityById(HealthUserContext ctx, String type, String id) throws Exception {
		return loadBaseEntityById(ctx, type, id, null);
	}
	public static <T> T loadBaseEntityById(HealthUserContext ctx, String type, String id, Consumer<T> enhancer) throws Exception {
		String key = type+":"+id;
	...
	public static <T extends BaseEntity> void appendLinkToUrl(HealthUserContext ctx, List<T> list,
			Function<T, String> makeFunc) {
		if (list == null || list.isEmpty()) {
			return;
		}
		list.forEach(it -> {
			it.addItemToValueMap(HealthBaseConstants.X_LINK_TO_URL, makeFunc.apply(it));
		});
	}

	public static <T extends BaseEntity> void appendLinkToUrl(HealthUserContext ctx, T obj, String url) {
		if (obj == null) {
			return;
		}
		obj.addItemToValueMap(HealthBaseConstants.X_LINK_TO_URL, url);
	}

	public static ButtonViewComponent addAction(HealthUserContext ctx, BaseEntity obj, String title, String code,
			String linkToUrl) {
	...
	public static void setAction(HealthUserContext ctx, BaseEntity obj, String title, String code, String linkToUrl) {
		ButtonViewComponent actionBtn = new ButtonViewComponent(title, null, code);
		actionBtn.setLinkToUrl(linkToUrl);
